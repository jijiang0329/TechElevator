...
public class Exercises {
    ...
    public String[] list2Array(List<String> stringList) {
        ...
        String[] result = stringList.toArray(strArray);
        return result;
    }
    ...
    public List<String> no4LetterWords(String[] stringArray) {
        ...
        for (String item : stringArray) {
            if (item.length() != 4) result.add(item);
        }
    ...
    ** INTERVIEW QUESTION **
    ...
    public List<String> fizzBuzzList(Integer[] integerArray) {
        ...
        for (int item : integerArray) {
            if (item % 3 == 0 && item % 5 == 0) result.add("FizzBuzz");
            else if (item % 3 == 0) result.add("Fizz");
            else if (item % 5 == 0) result.add("Buzz");
            else result.add(Integer.toString(item));
        }
        return result;
    }
    ...
    public static List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
        ...
        int theShorterSize = Math.min(listOne.size(), listTwo.size());
        int i;

        for (i = 0; i < theShorterSize; i++) {
            result.add(2 * i, listOne.get(i));
            result.add(2 * i + 1, listTwo.get(i));
        }
        if (i < listTwo.size()) {
            for (int j = i; j < listTwo.size(); j++) {

                result.add(i + j, listTwo.get(j));

            }
        } else if (i < listOne.size()) {
            for (int j = i; j < listOne.size(); j++) {

                result.add(i + j, listOne.get(j));
