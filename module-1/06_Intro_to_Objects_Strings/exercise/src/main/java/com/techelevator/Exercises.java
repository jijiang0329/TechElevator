...
public class Exercises {
	...
	public String twoChar(String str, int n) {
		if (n < 0) return str.substring(0,2);
	...
	public boolean hasBad(String str) {
		return str.length()>4? str.substring(0,4).contains("bad") : str.contains("bad");
	}
	...
	public String frontTimes(String str, int n) {
		...
		return str.length() > 3? str.substring(0,3).repeat(n) : str.repeat(n);
	}
	...
	public int countXX(String str) {
		...
		for(int j = 0; j< strArray.length; j++) {
			for (int i = 0; i < strArray[j].length() -1; i++) {
				String subStr = strArray[j].substring(i, i+2);
				if (subStr.contains("xx"))
					counter++;
			}
		}
